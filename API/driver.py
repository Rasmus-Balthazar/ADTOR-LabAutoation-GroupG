...
class RealMicrocontrollerService:
    ...
    def set_state(self, 
        ...
        log.info(f"Setting state: A={stateB},{speedB},{dirB} time={stepTime}")
        log.info(f"Setting state: A={stateC},{speedC},{dirC} time={stepTime}")
        try:
            self.comm.send("kStep", 
                           stateA, speedA, dirA, 
                           stateB, speedB, dirB, 
                           stateC, speedC, dirC, 
                           stepTime) # fails, number of argument must match the number of arguments
            msg = self.comm.receive()
            log.info(msg)
